...
for root, _, files in os.walk(LOG_DIR):
    for file in files:
        ...
        if parsed:
            results[(dataset, label)]["val_auc"].append(parsed["val_auc"])
            results[(dataset, label)]["val_ap"].append(parsed["val_ap"])
...
for (dataset, label), vals in results.items():
    epoch_stats[(dataset, label)] = {
        "auc_mean": np.nanmean(pad(vals["val_auc"]), axis=0),
        "auc_std": np.nanstd(pad(vals["val_auc"]), axis=0),
        "ap_mean": np.nanmean(pad(vals["val_ap"]), axis=0),
        "ap_std": np.nanstd(pad(vals["val_ap"]), axis=0),
    }
...
def plot_single(metric, dataset_key, display_name):
    fig, ax = plt.subplots(figsize=(5.5, 4)) # Rozmiar pojedynczego wykresu
    # ax.set_title(display_name, fontweight='bold', pad=10)
    
    dataset_keys = [(d, l) for (d, l) in epoch_stats.keys() if d == dataset_key]
    
    if not dataset_keys:
        # Placeholder jeśli brakuje danych
    ...
    # Zapis bez legendy w środku
    out = f"figures/{dataset_key}_{metric}.pdf"
    plt.savefig(out)
    plt.close(fig)
    print(f"[OK] Saved {out}")

def export_standalone_legend():
    """Generuje osobny plik PDF zawierający wyłącznie wspólną legendę."""
    fig_leg = plt.figure(figsize=(8, 0.5))
    ax_leg = fig_leg.add_subplot(111)
    ax_leg.axis('off')
    
    # Tworzymy fałszywe linie do wygenerowania uchwytów legendy
    handles = []
    labels = []
...
for metric in ["auc", "ap"]:
    for d_key, d_name in DATASETS_CONFIG.items():
        plot_single(metric, d_key, d_name)
