...
class LinkPredictor(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, max_dist=10):
        super().__init__()
        # DRNL max label = 1 + (d//2) + (d//2) * (d//2 + d%2 - 1)
        half = max_dist // 2
        mod  = max_dist % 2
        # Maksymalna etykieta gdy obie odległości są max_dist
        # d_sum = max_dist + max_dist = 2*max_dist
        d_sum = 2 * max_dist
        over2 = d_sum // 2
        mod2  = d_sum % 2
        max_label = int(1 + max_dist + over2 * (over2 + mod2 - 1))
        num_labels = max_label + 2  # +1 dla 0-indeksowania, +1 margines
        self.label_emb = Embedding(num_labels, in_channels, padding_idx=0)
    ...
    def forward(self, z, edge_index, local_src, local_dst):
        batch_labels = labeling.drnl_node_labeling(
