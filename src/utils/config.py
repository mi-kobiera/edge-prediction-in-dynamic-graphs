...
class DatasetConfig(BaseModel):
    ...
    negative_sampling_ratio: int = Field(1, ge=1, description="Number of negative samples per positive edge")
...
class ExperimentConfig(BaseModel):
    ...
    device: Literal["cpu", "cuda", "mps"] = Field("cuda") #TODO dla mps: RuntimeError: indices should be either on cpu or on the same device as the indexed tensor (cpu)
